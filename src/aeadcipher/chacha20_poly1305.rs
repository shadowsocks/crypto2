...
use crate::streamcipher::XChacha20;

macro_rules! impl_cipher_with_poly1305 {
    ($(#[$attr:meta])* => $name:tt, $cipher:tt) => {
        $(#[$attr])*
        #[derive(Clone)]
        pub struct $name {
            cipher: $cipher,
        ...
        impl $name {
            pub const KEY_LEN: usize   = $cipher::KEY_LEN;   // 32 bytes
            pub const BLOCK_LEN: usize = $cipher::BLOCK_LEN; // 64 bytes
            pub const NONCE_LEN: usize = $cipher::NONCE_LEN; // 12 bytes
            ...
            pub fn encrypt_slice_detached(&self, nonce: &[u8], aad: &[u8], plaintext_in_ciphertext_out: &mut [u8], tag_out: &mut [u8]) {
                ...
                self.cipher.encrypt_slice(1, &nonce, plaintext_in_ciphertext_out);

                let mut poly1305 = {
                    let mut keystream = [0u8; Self::BLOCK_LEN];
                    // NOTE: 初始 BlockCounter = 0;
                    self.cipher.encrypt_slice(0, &nonce, &mut keystream);
            ...
            pub fn decrypt_slice_detached(&self, nonce: &[u8], aad: &[u8], ciphertext_in_plaintext_out: &mut [u8], tag_in: &[u8]) -> bool {
                ...
                if is_match {
                    // NOTE: 初始 BlockCounter = 1;
                    self.cipher.decrypt_slice(1, &nonce, ciphertext_in_plaintext_out);
                }
...
impl_cipher_with_poly1305!{
/// XChaCha20 and Poly1305 for IETF Draft
/// 
