...
impl Blake2s {
    ...
    pub fn finalize(mut self) -> [u8; Self::H_MAX] {
        ...
        unsafe {
            // Last Block
            self.counter = self.counter.wrapping_add(rlen);
    ...
    pub fn oneshot_hash<T: AsRef<[u8]>>(iv: [u32; 8], data: T) -> [u8; Self::H_MAX] {
        ...
        unsafe {
            ...
            let n = ilen / Blake2s::BLOCK_LEN;
            let r = ilen % Blake2s::BLOCK_LEN;

            let chunks: &[u8];
            let last_block_len: usize;

            let mut last_block = [0u8; Blake2s::BLOCK_LEN];
            ...
            if r > 0 {
                ...
                if n > 0 {
                    ...
                    debug_assert!(ilen >= Blake2s::BLOCK_LEN);
                    let clen = ilen - Blake2s::BLOCK_LEN;
                    chunks = &data[..clen];
                    last_block.copy_from_slice(&data[clen..]);
                    last_block_len = Blake2s::BLOCK_LEN;
                } else {
            ...
            for chunk in chunks.chunks_exact(Blake2s::BLOCK_LEN) {
                counter = counter.wrapping_add(Blake2s::BLOCK_LEN as u64);
